...
def nntr_pretraining(
        train_uncertainties=False,
        detector_type="idealized_detector",
        model_name="v1_test"
        ) -> (TrainingBase, simpleMetricsCallback):
    """ Set the model, optimizer and loss function for the training
    Also return metrics callbacks.
    """
    assert detector_type and os.path.isdir(f"./nntr_data/{detector_type}"), \
        "Detector type not found in directory 'nntr_data'"
    os.makedirs(f"./nntr_models/{detector_type}/{model_name}", exist_ok=True)
    os.system(f"rm -rf ./nntr_models/{detector_type}/{model_name}/Output")

    train = TrainingBase(
        inputDataCollection=f"./nntr_data/{detector_type}/Training/dataCollection.djcdc",
        outputDir=f"./nntr_models/{detector_type}/{model_name}/Output",
        valdata=f"./nntr_data/{detector_type}/Training/dataCollection.djcdc",
        takeweights=""
    )
    ...
    if not train.modelSet():
        ...
        train.compileModel(
            learning_rate=1e-3,
            loss=nntr_L2_distance(train_uncertainties=train_uncertainties))
