...
class TrainData_TrackReco(TrainData):
    ...
    def convertFromSourceFile(self, filename, weightobjects, istraining):
        ...
        for keys in keys :
            ...
            # Ignore the "layerType == 0 hits as they record the hits in the
            # absorber layer
        ...
        offsets = np.cumsum(np.array(np.append(0, np.array(
                        ak.to_list(ak.num(feature_array, axis=1))))))
        ...
        for keys in keys :
            d = self.extract_to_array(filename=filename, key=prefix+keys)/1000
        ...
        # Deletes empty events which can be due to both photons missing the
        # detector
        ...
        print("Truth array[0] before adding norm:\n", truth_array[0])
        norm_p1 = np.sqrt(np.sum(truth_array[:,0:3]**2, axis=-1))
        norm_p2 = np.sqrt(np.sum(truth_array[:,6:9]**2, axis=-1))
        ...
        # Normalize the momentum three vector for easier learning (values
        # closer to [0,1])
        truth_array[:,0:3] = truth_array[:,0:3] \
            /(np.array(ak.to_list(norm_p1+1E-10))[..., np.newaxis])
        truth_array[:,7:10] = truth_array[:,7:10] \
            /(np.array(ak.to_list(norm_p2+1E-10))[..., np.newaxis])
        ...
        print("Truth array[0] ater adding norm:\n", truth_array[0])

        # Return a list of feature arrays, a list of truth arrays (and
        # optionally a list of weight arrays)
