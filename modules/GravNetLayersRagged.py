...
def AccumulateKnnSumw(distances,  features, indices, mean_and_max=False):
    ...
    f, midx = AccumulateKnn(distances,  features,
                            indices, mean_and_max=mean_and_max)
...
def AccumulateLinKnnSumw(weights,  features, indices, mean_and_max=False):
    ...
    origshape = features.shape[1]
    features = tf.concat([features, tf.ones_like(features[:, 0:1])], axis=1)
    f, midx = AccumulateLinKnn(
        weights,  features, indices, mean_and_max=mean_and_max)
...
def select_threshold_with_backgather(score, threshold, row_splits):
    ...
    for i in tf.range(tf.shape(row_splits)[0]-1):
        ...
        # just in the very very inlikley case the above gives>1 results
        minidx = keeprange[minidxlocal_rs][0]
    ...
    sel = tf.expand_dims(tf.concat(allkeepidxs, axis=0),
                         axis=1)  # standard tf index format
    newrs = tf.concat(newrs, axis=0)

    # sanity check
    selglidxs = SelectWithDefault(
        sel, tf.expand_dims(glidxs, axis=1), 0)[:, :, 0]
    mess = "select_threshold_with_backgather: assert indices " + \
        str(selglidxs.shape)+" vs "+str(sel.shape)
...
class AssertEqual(tf.keras.layers.Layer):
    def call(self, inputs):
        assert len(inputs) == 2
        tf.assert_equal(tf.shape(inputs[0]), tf.shape(
            inputs[1]), "shape not equal")
...
class CondensateToIdxs(LayerWithMetrics):
    ...
    def call(self, inputs):
        ...
        if self.active:
            ...
            d = d * t_d
            # abs is safety guard as the constraint does not seem to be inf
            t_b = tf.abs(self.dyn_t_b).numpy()[0]
            ...
            if False:
                for i in tf.range(rs.shape[0]-1):
                    with tf.control_dependencies([pred_sid]):
                        ...
                        nseg_a = tf.reduce_sum(
                            tf.cast(tf.unique(tf.squeeze(check))[0], dtype='int32') * 0 + 1)
            ...
            pred_sid = tf.ragged.row_splits_to_segment_ids(
                rs, out_type=tf.int32)
...
class CleanCondensations(LayerWithMetrics):
    ...
    def call(self, inputs):
        ...
        pred_sid = tf.where(
            score[:, 0] < self.threshold, -1, pred_sid[:, 0])[:, tf.newaxis]
        asso = tf.where(score[:, 0] < self.threshold,
                        tf.range(tf.shape(asso)[0]), asso)
...
class CreateIndexFromMajority(tf.keras.layers.Layer):
    ...
    def __init__(self, min_threshold=0.6, ignore: int = -10, assign_else: int = -1, active=True, **kwargs):
        ...
        raise ValueError(
            "TBI. There seems to be an issue somewhere, needs to be investigated before using this layer")
    ...
    def raw_call(inputs, min_threshold, ignore, assign_else, row_splits):
        ...
        fracmax = tf.cast(maxcount, dtype='float32') / \
            tf.cast(ntot, dtype='float32')
...
class PrintMeanAndStd(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        try:
            if self.print_mean:
                tf.print(self.name, 'mean', tf.reduce_mean(
                    inputs), summarize=100)
            if self.print_std:
                tf.print(self.name, 'std', tf.math.reduce_std(
                    inputs), summarize=100)
            if self.print_shape:
...
class ConditionalBatchNorm(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        x0 = self.bn0(x0)
        x1 = self.bn1(x1)
        # remove the added dimension to make it 1D again
...
class GooeyBatchNorm(LayerWithMetrics):
    ...
    def _update_viscosity(self, training):
        if self.fluidity_decay > 0:
            newvisc = self.viscosity + \
                (self.max_viscosity - self.viscosity)*self.fluidity_decay
            newvisc = tf.keras.backend.in_train_phase(
                newvisc, self.viscosity, training=training)
    ...
    def call(self, inputs, training=None):
        ...
        if self.trainable and not self.learn:
            ...
            update = self._calc_update(
                self.mean, newmean, training, ismean=True)
            tf.keras.backend.update(self.mean, update)

            # use the actual mean here, not self.mean
            newvar = tf.math.reduce_std(
                x-currentmean, axis=0, keepdims=True)  # FIXME
            update = self._calc_update(
                self.variance, newvar, training, ismean=False)
            ...
            currentmean = tf.where(tf.math.is_finite(
                currentmean), currentmean, self.mean)

            self.add_loss(10.*(1.01-self.viscosity) *
                          tf.reduce_mean(tf.abs(self.mean-currentmean)))

            newvar = tf.math.reduce_std(x-currentmean, axis=0, keepdims=True)
            newvar = tf.stop_gradient(newvar)
            # protect against empty batches
            newvar = tf.where(tf.math.is_finite(newvar), newvar, self.variance)

            self.add_loss(10.*(1.01-self.viscosity) *
                          tf.reduce_mean(tf.abs(self.variance-newvar)))
...
class ScaledGooeyBatchNorm2(LayerWithMetrics):
    ...
    def _m_mean(self, x, mask):
        ...
        norm = tf.abs(tf.reduce_sum(
            mask, axis=0, keepdims=True)) + self.epsilon
        return tf.math.divide_no_nan(x, norm)

    def _calc_mean_and_protect(self, x, mask, default):
        x = self._m_mean(x, mask)
        # protect against empty batches
        x = tf.where(tf.math.is_finite(x), x, default)
        return x
    ...
    def call(self, inputs, training=None):
        if isinstance(inputs, list):
            x_in, cond = inputs
            # make sure it's ones and zeros
            cond = tf.where(cond > 0.5, tf.ones_like(cond), 0.)
        else:
            x_in = inputs
...
class ProcessFeatures(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        if not self.newformat:
            ...
            fdict['recHitTime'] = tf.nn.relu(
                fdict['recHitTime'])/10.  # remove -1 default
            allf = []
            for k in fdict:
                allf.append(fdict[k])
...
class NeighbourCovariance(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        cov, means = NeighbourCovariance.raw_call(
            coordinates, distsq, features, n_idxs)
    ...
    def __init__(self, hidden_nodes=[32, 32, 32], **kwargs):
        ...
        for i in range(len(hidden_nodes)):
            with tf.name_scope(self.name + "/1/" + str(i)):
                self.hidden_dense.append(tf.keras.layers.Dense(
                    hidden_nodes[i], activation='elu'))
...
class ApproxPCA(tf.keras.layers.Layer):
    ...
    def __init__(self, size='small',
                 base_path=os.environ.get(
                     'HGCALML') + '/HGCalML_data/pca/pretrained/',
        ...
        assert size.lower() in [
            'small', 'medium', 'large'], "size must be 'small', 'medium', or 'large'!"
    ...
    def get_config(self):
        ...
        config = dict(list(base_config.items()) +
                      list(init_config.items()) + list(self.config.items()))
        return config
        ...
        for i in range(len(nodes) + 1):
            with tf.name_scope(self.name + "/1/" + str(i)):
                ...
                if i == 0:
                    ...
                    # Not sure if I need the batch dimension
                    input_dim = [None, self.nC**2]
                else:
                ...
                if i == len(nodes):
                    ...
                    layer = tf.keras.layers.Dense(
                        units=output_dim, trainable=self.train_layers)
                else:
                    output_dim = nodes[i]
                    layer = tf.keras.layers.Dense(
                        units=output_dim, activation='elu', trainable=self.train_layers)
...
class LocalDistanceScaling (LayerWithMetrics):
    ...
    def call(self, inputs):
        ...
        self.add_prompt_metric(tf.math.reduce_std(
            scale), self.name+'_var_dist_scale')
        return newdist
...
class CreateGlobalIndices(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        add = tf.expand_dims(
            tf.range(tf.shape(inputs)[0], dtype='int32'), axis=1)
        return ins+add
...
class WeightedNeighbourMeans(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        nweights = SelectWithDefault(nidx, weights, 0.)[
            :, :, 0]  # remove last 1 dim
...
class WeightFeatures(tf.keras.layers.Layer):
    def __init__(self,
        ...
        with tf.name_scope(self.name + "/1/"):
            self.weight_dense = tf.keras.layers.Dense(
                1, activation='tanh', bias_initializer='ones')
    ...
    def call(self, inputs):
        ...
        # normliase such that with chosen bias initializer weights are 1
        weight = self.weight_dense(wfeat)/0.761594156
...
class RecalcDistances(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        dist = tf.reduce_sum(
            (ncoords - tf.expand_dims(coords, axis=1))**2, axis=2)
        return dist
...
class SelectFromIndicesWithPad(tf.keras.layers.Layer):
    ...
    def __init__(self, default=0., **kwargs):
        ...
        if 'dynamic' in kwargs:
            ...
            super(SelectFromIndicesWithPad, self).__init__(
...
class MultiBackScatter(tf.keras.layers.Layer):
    ...
    def raw_call(x, scatters):
        ...
        for k in range(len(scatters)):
            ...
            # cast is needed because keras layer out dtypes are not really working
            shape = tf.concat(
                [tf.expand_dims(V, axis=0), tf.shape(x)[1:]], axis=0)
...
class MultiBackScatterOrGather(tf.keras.layers.Layer):
    def __init__(self, default: float = 0., **kwargs):
        ...
        if 'dynamic' in kwargs:
            ...
            super(MultiBackScatterOrGather, self).__init__(
                dynamic=False, **kwargs)
            ...
            if isinstance(scatters[l], list):
                ...
                if default:
                    x = tf.tensor_scatter_nd_update(
                        tf.zeros(shape, x.dtype)+default, scidx, x)
                else:
                    x = tf.scatter_nd(scidx, x, shape)
            else:
...
class KNN(LayerWithMetrics):
    def __init__(self, K: int, radius=-1.,
        ...
        if isinstance(radius, int):
            radius = float(radius)
        self.radius = radius
        assert (isinstance(radius, str) and radius ==
                'dynamic') or isinstance(radius, float)
    ...
    def update_dynamic_radius(self, dist, training=None):
        ...
        update = tf.math.reduce_max(tf.sqrt(dist))*1.05
...
class SortAndSelectNeighbours(tf.keras.layers.Layer):
    ...
    def raw_call(distances, nidx, K, radius, sort, incr_sorting_score, keep_self=True):
        ...
        if tf.shape(incr_sorting_score)[1] is not None and tf.shape(incr_sorting_score)[1] == 1:
            incr_sorting_score = SelectWithDefault(
                nidx, incr_sorting_score, 0.)[:, 0]

        # make sure the -1 end up at the end
        tfssc = tf.where(nidx < 0, 1e9, incr_sorting_score)
        if keep_self:
            # make sure 'self' remains in first place
            tfssc = tf.concat(
                [tf.reduce_min(tfssc[:, 1:], axis=1, keepdims=True)-1., tfssc[:, 1:]], axis=1)

        sorting = tf.argsort(tfssc, axis=1)

        # _nd(nidx,sorting,batch_dims=1)
        snidx = tf.gather(nidx, sorting, batch_dims=1)
        ...
        return sdist, tf.cast(snidx, tf.int32)
    ...
    def call(self, inputs):
        ...
        if self.return_backscatter:
            ...
            for i in range(len(row_splits)-1):
                thissel = allidxs[row_splits[i]:row_splits[i+1]
                                  ][score[row_splits[i]:row_splits[i+1]] > self.threshold]
            ...
            sel, allbackgather, newrs = select_threshold_with_backgather(
                score, self.threshold, row_splits)

        if self.print_reduction:
            print(self.name, ' reduction from ', int(
                row_splits[-1]), ' to ', int(newrs[-1]), ': ', float(row_splits[-1])/float(newrs[-1]))

        self.add_prompt_metric(tf.cast(newrs[-1], dtype='float32')/tf.cast(
            row_splits[-1], dtype='float32'), self.name+'_reduction')
...
class EdgeCreator(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        if self.addself:
            edges = selffeat - \
                SelectWithDefault(inputs[0][:, :], inputs[1], -1.)
        else:
            edges = selffeat - \
                SelectWithDefault(inputs[0][:, 1:], inputs[1], -1.)
        return edges
...
class EdgeSelector(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        indices = tf.sort(indices, axis=1, direction="DESCENDING")
        sindxs = tf.concat([nidx[:, 0:1], indices], axis=1)
        # to get the output shapes defined
        sindxs = tf.reshape(sindxs, [-1, nidx.shape[1]])
        # old
        # add self score always 1
        score = tf.concat([tf.ones_like(score[:, 0:1, :]), score], axis=1)
...
class GroupScoreFromEdgeScores(tf.keras.layers.Layer):
    def __init__(self, den_offset=0., **kwargs):
        ...
        if 'dynamic' in kwargs:
            ...
            super(GroupScoreFromEdgeScores, self).__init__(
                dynamic=False, **kwargs)
...
class NeighbourGroups(LayerWithMetrics):
    def __init__(self,
        ...
        if threshold is None:
            ...
            assert purity_min_target is None and efficiency_min_target is None
        # make this a variable
        with tf.name_scope(self.name + "/1/"):
            self.threshold = tf.Variable(
                initial_value=self.initial_threshold, trainable=False, dtype='float32')
    ...
    def _update_threshold(self, purity, efficiency, training=None):
        ...
        updated_thresh = tf.keras.backend.in_train_phase(
            update, self.threshold, training=training)
    ...
    def call(self, inputs,  training=None):
        ...
        if row_splits.shape[0] is None:
            ...
            for i in range(tf.shape(row_splits)[0] - 1):
                a = tf.argsort(
                    score[row_splits[i]:row_splits[i+1]], axis=0, direction='DESCENDING')
    ...
    def compute_output_shape(self, input_shapes):
        ...
        if self.return_backscatter:
            # first dim omitted
            return nidxs, score, [(None, 1), score], row_splits
        else:
...
class SoftPixelCNN(tf.keras.layers.Layer):
    def __init__(self, length_scale_momentum=0.01, mode: str = 'onlyaxes', subdivisions: int = 3, **kwargs):
        ...
        with tf.name_scope(self.name + "/1/"):
            self.length_scale = tf.Variable(
                initial_value=1., trainable=False, dtype='float32')
    ...
    def create_offsets(D, subdivision):
        length_scale = 1
        temp = [np.linspace(-length_scale, length_scale, subdivision)
                for _ in range(D)]
        ...
        onlyaxis = np.concatenate(
            [onlyaxis, b[np.count_nonzero(b, axis=-1) == 1]], axis=0)
        return a, b, onlyaxis
    ...
    def call(self, inputs, training=None):
        ...
        if self.trainable:
            tf.keras.backend.update(self.length_scale,
                                    tf.keras.backend.in_train_phase(
                                        self.length_scale *
                                        (1.-self.length_scale_momentum) + self.length_scale_momentum *
                                        2.*tf.reduce_mean(tf.sqrt(distsq+1e-6)),
                                        self.length_scale,
                                        training=training)
                                    )
        ...
        for o in self.offsets:
            ...
            f, _ = AccumulateKnn(self.length_scale*distancesq,
                                 features, neighbour_indices)  # V x F
            f = f[:, :self.nfeat]
...
class RaggedGravNet(LayerWithMetrics):
    def __init__(self,
        ...
        with tf.name_scope(self.name + "/3/"):
            self.output_feature_transform = tf.keras.layers.Dense(
                self.n_filters, activation='relu')  # changed to relu
    ...
    def update_dynamic_radius(self, dist, training):
        ...
        # can be inverted for performance TBI
        update = tf.reduce_max(lindist)*1.05
        mean_dist = tf.reduce_mean(lindist)
        # receptive field ends at 1.
        low_update = tf.where(update > 2., 2., update)
        # safety setting to not loose all neighbours
        update = tf.where(low_update > 2.*mean_dist, low_update, 2.*mean_dist)
        update += 1e-3
    ...
    def create_output_features(self, x, neighbour_indices, distancesq):
        ...
        prev_feat = features
        features = self.collect_neighbours(
            features, neighbour_indices, distancesq)
    ...
    def priv_call(self, inputs, training=None):
        ...
        neighbour_indices, distancesq, sidx, sdist = self.compute_neighbours_and_distancesq(
            coordinates, row_splits, training)
        ...
        outfeats = self.create_output_features(
            x, neighbour_indices, distancesq)
        if self.return_self:
    ...
    def call(self, inputs):
        ...
        for di in range(len(self.kernel_coord_dense)):
            ...
            for i in range(coord.shape[-1]):
                ...
                self.add_prompt_metric(
                    meancoord, self.name+'_coord_add_mean_'+str(di)+'_'+str(i))
                self.add_prompt_metric(tf.math.reduce_std(
                    refcadd[:, i]-meancoord), self.name+'_coord_add_var_'+str(di)+'_'+str(i))
            ...
            acc, _ = AccumulateKnn(
                10.*dist, passfeat, nidx, mean_and_max=False)
...
class DynamicDistanceMessagePassing(tf.keras.layers.Layer):
    ...
    def __init__(self, n_feature_transformation,
        ...
        for i in range(len(self.n_feature_transformation)):
            with tf.name_scope(self.name + "/5/" + str(i)):
                self.dist_mod_dense.append(tf.keras.layers.Dense(
                    1, activation='sigmoid'))  # restrict variations a bit
            with tf.name_scope(self.name + "/6/" + str(i)):
                self.feature_tranformation_dense.append(tf.keras.layers.Dense(
                    self.n_feature_transformation[i], activation='relu'))
    ...
    def build(self, input_shapes):
        ...
        for i in range(1, len(self.dist_mod_dense)):
            with tf.name_scope(self.name+"/5/"+str(i)):
                self.dist_mod_dense[i].build(
                    (input_shape[0], input_shape[1]+self.n_feature_transformation[i-1]*2))
    ...
    def create_output_features(self, x, neighbour_indices, distancesq):
        ...
        for i in range(len(self.n_feature_transformation)):
            if i == 0:
                ...
                scale = 10. * \
                    self.dist_mod_dense[i](tf.concat([x, features], axis=-1))
            distancesq *= scale
...
class MessagePassing(tf.keras.layers.Layer):
    ...
    def __init__(self, n_feature_transformation,
        ...
        for i in range(len(self.n_feature_transformation)):
            with tf.name_scope(self.name + "/5/" + str(i)):
                self.feature_tranformation_dense.append(tf.keras.layers.Dense(
                    self.n_feature_transformation[i], activation='relu'))  # restrict variations a bit
    ...
    def collect_neighbours(self, features, neighbour_indices):
        ...
        f, _ = AccumulateLinKnn(
            tf.cast(neighbour_indices*0, tf.float32)+1.,  features, neighbour_indices)
        return f
...
class DistanceWeightedMessagePassing(tf.keras.layers.Layer):
    ...
    def collect_neighbours(self, features, neighbour_indices, distancesq):
        ...
        if self.sumwnorm:
            f, _ = AccumulateKnnSumw(
                10.*distancesq,  features, neighbour_indices, mean_and_max=True)
        else:
            f, _ = AccumulateKnn(10.*distancesq,  features, neighbour_indices)
        return f
...
class DistanceWeightedAttentionMP(DistanceWeightedMessagePassing):
    def __init__(self,
        ...
        for i in range(len(self.n_feature_transformation)):
            with tf.name_scope(self.name + "/6/" + str(i)):
                self.attention_dense.append(
                    tf.keras.layers.Dense(K, activation='sigmoid'))

        with tf.name_scope(self.name + "/7/"):
            self.pos_emb = tf.keras.layers.Dense(
                pos_embedding, activation=activation)
    ...
    def build(self, input_shapes):
        ...
        for i in range(1, len(self.feature_tranformation_dense)):
            total_feat = 2 * \
                self.n_feature_transformation[i -
                                              1] + self.K * self.pos_emb.units
            with tf.name_scope(self.name + "/6/" + str(i)):
...
class EdgeConvStatic(tf.keras.layers.Layer):
    ...
    def build(self, input_shapes):
        ...
        with tf.name_scope(self.name + "/5/" + str(0)):
            self.feature_tranformation_dense[0].build(
    ...
    def call(self, inputs):
        ...
        if self.select_neighbours:
            ...
            neighfeat = tf.where(tf.expand_dims(
                nidx, axis=2) < 0, 0., neighfeat)  # zero pad
...
class SingleLocalGravNetAttention(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        with tf.name_scope(self.name + "/1/"):
            self.key_dense = tf.keras.layers.Dense(
                self.n_keys, activation='tanh')  # better behaved
        with tf.name_scope(self.name + "/2/"):
        ...
        if self.n_pre_keys > 0:
            with tf.name_scope(self.name + "/4/"):
                self.pre_key_dense = tf.keras.layers.Dense(
                    self.n_pre_keys, activation='elu')
    ...
    def call(self, inputs):
        ...
        if self.n_pre_keys > 0:
            ...
            n_keys = tf.concat(
                [n_keys, tf.stop_gradient(distsq[..., tf.newaxis])], axis=-1)
        ...
        # accumulateknn includes 1/d_k, so make the weights O(1) again
        att = att * d_k

        acc = AccumulateLinKnn(att, values, nidx, force_tf=False, mean_and_max=True)[
            0]  # [ V x 2F] -> one head
...
class LocalGravNetAttention(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        for i in range(self.n_heads):
            with tf.name_scope(self.name + "/1/"+str(i)):
                self.heads.append(SingleLocalGravNetAttention(
                    n_keys, n_values, n_pre_keys=n_pre_keys))
