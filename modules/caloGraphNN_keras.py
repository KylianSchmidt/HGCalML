...
class GravNet(keras.layers.Layer):
    def __init__(self, n_neighbours, n_dimensions, n_filters, n_propagate, name,
        ...
        self.input_spatial_transform = keras.layers.Dense(
            n_dimensions, name=name+'_S', kernel_initializer=coordinate_kernel_initializer, activation=coordinate_activation)
        self.output_feature_transform = keras.layers.Dense(
            n_filters, activation='tanh', name=name+'_Fout', kernel_initializer=other_kernel_initializer)

        self._sublayers = [self.input_feature_transform,
                           self.input_spatial_transform, self.output_feature_transform]
    ...
    def build(self, input_shape):
        ...
        self.output_feature_transform.build(
            (input_shape[0], input_shape[1], input_shape[2] + self.input_feature_transform.units * 2))

        self.message_parsing_distance_weights = []
        for i in range(len(self.message_passing_layers)):
            l = self.message_passing_layers[i]
            l.build((input_shape[0], input_shape[1],
                    input_shape[2] + self.n_propagate * 2))
    ...
    def call(self, x):
        ...
        if self.masked_coordinate_offset is not None:
            ...
            coordinates = tf.where(sel_mask > 0., coordinates, tf.zeros_like(
                coordinates)-self.masked_coordinate_offset)
    ...
    def collect_neighbours(self, coordinates, x, mask):
        ...
        batch_indices = tf.tile(
            batch_range, [1, n_vertices, self.n_neighbours - 1, 1])  # (B, V, N-1, 1)
        vertex_indices = tf.expand_dims(
            neighbour_indices, axis=3)  # (B, V, N-1, 1)
        ...
        for i in range(len(self.message_passing_layers)+1):
            if i:
                features = self.message_passing_layers[i -
                                                       1](tf.concat([features, x], axis=-1))
...
class GarNet(keras.layers.Layer):
    def __init__(self, n_aggregators, n_filters, n_propagate, name,  **kwargs):
        ...
        self.input_feature_transform = keras.layers.Dense(
            n_propagate, name=name+'_FLR')
        self.aggregator_distance = keras.layers.Dense(
            n_aggregators, name=name+'_S')
        self.output_feature_transform = keras.layers.Dense(
            n_filters, activation='tanh', name=name+'_Fout')

        self._sublayers = [self.input_feature_transform,
                           self.aggregator_distance, self.output_feature_transform]
    ...
    def call(self, x):
        ...
        features_with_edge_weights = tf.concat(
            [features, edge_weights], axis=-1)  # (B, V, F+S)

        # vertices -> aggregators
        edge_weights_trans = tf.transpose(
            edge_weights, perm=(0, 2, 1))  # (B, S, V)
        ...
        aggregated = tf.concat(
            [aggregated_max, aggregated_mean], axis=-1)  # (B, S, 2*(F+S))

        # aggregators -> vertices
        updated_features = self.apply_edge_weights(
            aggregated, edge_weights)  # (B, V, 2*S*(F+S))

        updated_features = tf.concat(
            [x, updated_features, edge_weights], axis=-1)  # (B, V, X+2*S*(F+S)+S)
